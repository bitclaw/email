...
export class SmtpProvider extends BaseEmailProvider {
    ...
    async send(options) {
        ...
        if (!transporter) {
            return err(EMAIL_ERROR_CODES.PROVIDER_NOT_CONFIGURED, 'SMTP configuration is missing');
        }
        ...
        try {
            ...
            if (isConnectionRefused) {
                return err(EMAIL_ERROR_CODES.CONNECTION_FAILED, `SMTP connection refused at ${this.config.smtp?.host}:${this.config.smtp?.port}`, cause);
            }
            if (code === 'EAUTH' || code === 'ESOCKET') {
                return err(EMAIL_ERROR_CODES.AUTH_FAILED, `SMTP authentication failed: ${cause.message}`, cause);
            }
            return err(EMAIL_ERROR_CODES.SEND_FAILED, `Failed to send email via SMTP: ${cause.message}`, cause);
        }
    }
    getProviderName() {
        return 'smtp';
